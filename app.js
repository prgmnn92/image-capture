...
const startUp = () => {
	...
	// colorPickerBg1.addEventListener('change', (event) => {

	// 	globalImage.setBackgroundColor(event.target.value.toString());
	// });

	setInterval(() => {
		captureXposText.value = captureXpos.value;
		captureYposText.value = captureYpos.value;
		globalImage.setColors(colorPickerBg1.value, colorPickerBg2.value, colorPickerText.value);
